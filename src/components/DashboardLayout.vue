  ...
  <v-layout :dir="isRtl ? 'rtl' : 'ltr'" class="dashboard-layout">
    ...
    >
      <v-list-item
        ...
        :subtitle="adminUser.email || 'مدير النظام'"
      ...
      >
        ...
        >
          <template v-slot:activator="{ props }">
            <v-btn icon color="grey-darken-2" class="mr-2" v-bind="props">
              <v-badge
                :color="notificationsCount > 0 ? 'error' : 'transparent'"
                :content="notificationsCount"
                :dot="notificationsCount > 0"
                :class="{ 'pulse-error': notificationsCount > 0 }"
                offset-x="5"
                offset-y="5"
              >
...
.v-list-item--active {
  background: linear-gradient(
    ...
    rgba(255, 202, 40, 0.02)
  ) !important;
  box-shadow: 0 4px 15px rgba(255, 202, 40, 0.05) !important;
  color: #ffca28 !important;
}
